...
_date_short_mon = _whole_word(r'\d{2} (Jun|Jan|Feb|Mar|May|Apr|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')
...
def dates_iso8601(text):
    '''Find tokens that are dates'''
    for match in _date_iso8601_pat.finditer(text):
        yield('date', match)

def dates_short_mon(text):
    '''Find tokens that are dates in short mon format'''
    for match in _date_short_mon.finditer(text):
        yield('date', match)
